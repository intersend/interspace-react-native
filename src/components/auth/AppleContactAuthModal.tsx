...
export default function AppleContactAuthModal({
  ...
  const handleSend = async () => {
    if (!value) {
      Alert.alert('Missing', `Please enter your ${strategy === 'email' ? 'email address' : 'phone number'}`);
      return;
    }

    try {
      setLoading(true);
      await sendVerificationCode(strategy, value);
      setStep('verify');
    } catch (err: any) {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (strategy === 'email') {
        (config as any).email = value;
      } else {
        (config as any).phoneNumber = value;
      }
  ...
  const renderContactStep = () => (
    <View style={styles.formContent}>
      <Text style={styles.title}>
        {strategy === 'email' ? 'Sign in with Email' : 'Sign in with Phone'}
      </Text>
      <AppleTextInput
        placeholder={strategy === 'email' ? 'Email Address' : 'Phone Number'}
        keyboardType={strategy === 'email' ? 'email-address' : 'phone-pad'}
        value={value}
      ...
      <View style={styles.modalContainer}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>
            {strategy === 'email' ? 'Email Sign In' : 'Phone Sign In'}
          </Text>
